...
var _ = Describe("costManagementAdapter", func() {
	...
	Describe("HealthCheck", func() {
		It("checks the receiver readiness endpoint", func() {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				Expect(r.Method).To(Equal(http.MethodGet))
				Expect(r.URL.Path).To(Equal(costReadyPath))
				w.WriteHeader(http.StatusOK)
			}))
			DeferCleanup(server.Close)

			client := newCostManagementClient(server.URL, "cost-token")

			Expect(client.healthCheck(context.Background())).To(Succeed())
		})

		It("reports non-successful receiver readiness responses", func() {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				w.WriteHeader(http.StatusServiceUnavailable)
			}))
			DeferCleanup(server.Close)
			...
			Expect(client.healthCheck(context.Background())).To(MatchError(ContainSubstring("returned 503")))
		})
	})
})
